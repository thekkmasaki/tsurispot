...
test.describe("セッション・セキュリティ", () => {
  ...
  test.describe("認証フロー完全性", () => {
    ...
    test("providers APIで返される Cognito の設定が正しい", async ({
      request,
    }) => {
      const res = await request.get("/api/auth/providers");
      const body = await res.json();

      expect(body.cognito).toBeDefined();
      expect(body.cognito.id).toBe("cognito");
      expect(body.cognito.type).toBe("oidc");

      expect(body.cognito.signinUrl).toMatch(
        /\/api\/auth\/signin\/cognito$/
      );
      expect(body.cognito.callbackUrl).toMatch(
