...
export const seaIkaTakoAptitude: Record<string, FishRegionalAptitude> = {
  aoriika: {
    ...
    prefectureOverrides: {
      ...
      iwate: 0,
      miyagi: 1,
      akita: 2, // 男鹿半島はSHIMANO公式が「東北の新アオリイカフィールド」と特集する秋イカの新興地（カンパリ・エギCOMに実績多数）
      yamagata: 1,
      fukushima: 1,
      ibaraki: 1,
    },
    rationale:
      "海面漁業生産統計のいか類漁獲量は長崎・山口・島根・佐賀など対馬暖流域が上位で、エギング実績も山陰・北陸・九州日本海側が突出。太平洋側は和歌山・高知・三重・静岡など黒潮域に実績が集中する。分布北限は東北で、青森・山形〜福島は境界域（温暖化で北上中の秋田・男鹿は秋イカが定着しつつあり普通評価）、北海道は実質分布しない。有明海の泥干潟と東京湾奥には実質不在。",
    sources: [
