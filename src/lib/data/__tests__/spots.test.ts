...
describe("spots data", () => {
  ...
  it("all spots should have required fields", () => {
    fishingSpots.forEach((s) => {
      ...
      expect(s.spotType).toMatch(/^(port|beach|rocky|river|pier|breakwater|surf|lake|pond)$/);
      expect(s.difficulty).toMatch(/^(beginner|intermediate|advanced|all)$/);
  ...
  it("全スポットのspotTypeが有効値", () => {
    const validTypes = ["port", "breakwater", "rocky", "river", "beach", "pier", "surf", "lake", "pond"];
    fishingSpots.forEach((s) => {
