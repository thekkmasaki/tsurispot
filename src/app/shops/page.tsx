...
const shopsData = filteredShops.map((s) => ({
  id: s.id, name: s.name, slug: s.slug, description: s.description,
  address: s.address, phone: s.phone, businessHours: s.businessHours,
  hasLiveBait: s.hasLiveBait, hasFrozenBait: s.hasFrozenBait,
  hasRentalRod: s.hasRentalRod, hasParking: s.hasParking,
  parkingDetail: s.parkingDetail, services: s.services,
  isPremium: s.isPremium, region: s.region, latitude: s.latitude,
  longitude: s.longitude, closedDays: s.closedDays, website: s.website,
  baitStock: s.baitStock, nearbySpotSlugs: s.nearbySpotSlugs,
...
const currentTopFish = currentGuide
  ? currentGuide.topFish.slice(0, 3).map((slug) => fishSpecies.find((f) => f.slug === slug)).filter(Boolean)
  : [];

// 地方別の店舗数を計算
const prefectureShopCounts = new Map<string, number>();
for (const shop of filteredShops) {
  const pref = shop.region.prefecture;
  prefectureShopCounts.set(pref, (prefectureShopCounts.get(pref) || 0) + 1);
}

// 主要都道府県（店舗数上位8件）
const topPrefectures = prefectures
  .map((p) => ({ ...p, count: prefectureShopCounts.get(p.name) || 0 }))
  .filter((p) => p.count > 0)
...
export default function ShopsListPage() {
  return (
    <>
      ...
      {/* Hero Header */}
      <div className="border-b bg-gradient-to-b from-blue-50/80 to-white dark:from-blue-950/20 dark:to-background">
          ...
          <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h1 className="flex items-center gap-2.5 text-2xl font-bold sm:text-3xl">
                <Store className="size-7 text-blue-600 sm:size-8" />
                釣具店・エサ店ガイド
              </h1>
              <p className="mt-1.5 text-sm text-muted-foreground sm:text-base">
                全国<span className="font-bold text-foreground">{shopCount}</span>件の釣具店・エサ店から、釣り場の近くで装備を揃えよう
              </p>
            </div>
      ...
      <div className="container mx-auto px-4 py-6 sm:py-8">
        ...
        {/* 都道府県別 全一覧（コンパクト版） */}
        <div id="all-prefectures" className="mb-10 scroll-mt-20">
          ...
          <div className="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {regionGroupOrder.map((regionName) => {
              ...
              return (
                <div key={regionName} className="rounded-lg border bg-card p-3">
                  <h3 className="mb-2 border-b pb-1.5 text-xs font-bold text-muted-foreground">
                    {regionName}
                  </h3>
          ...
          <Card className="border-primary/20 bg-gradient-to-br from-primary/5 to-transparent">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-lg text-green-700 dark:text-green-400">無料掲載</CardTitle>
                    <p className="text-2xl font-bold">¥0</p>
                  </CardHeader>
                  <CardContent>
                    <ul className="space-y-2">
                      {["店舗基本情報の掲載", "近くの釣りスポットとの連携表示", "Google検索への構造化データ配信", "エサ在庫の掲載（1日10回まで）"].map((text) => (
                ...
                <Card className="border-blue-300/50">
                  ...
                  <CardContent>
                    <ul className="space-y-2">
                      {["無料掲載の全機能", "検索結果での優先表示", "写真3枚まで掲載", "公式バッジ表示"].map((text) => (
                        <li key={text} className="flex items-start gap-2 text-sm">
                ...
                <Card className="border-amber-300/50">
                  ...
                  <CardContent>
                    <ul className="space-y-2">
                      {["ベーシックの全機能", "写真20枚・店主メッセージ", "クーポン配信機能", "スポットページでの商品PR"].map((text) => (
                        <li key={text} className="flex items-start gap-2 text-sm">
