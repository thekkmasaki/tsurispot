...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    siteName: "ツリスポ",
    images: [{
      url: `/api/og?title=${encodeURIComponent("ショアジギング入門ガイド")}&emoji=${encodeURIComponent("⚡")}`,
      width: 1200,
      height: 630,
    }],
  },
  ...
  return (
    <>
      ...
      <main className="container mx-auto max-w-3xl px-4 py-8 sm:py-12">
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  {jiggingSpots.map((spot) => {
                    ...
                    <Link prefetch={false}
                      ...
                      className="group flex items-center gap-2 rounded-lg border p-3 transition-colors hover:border-primary"
                      title={`${spot.name}（${spot.region.prefecture}のショアジギングポイント）`}
                    >
        ...
        {/* ショアジギングにおすすめの道具（アフィリエイト） */}
        <section className="mt-8 sm:mt-10">
          <ProductList
            products={getProductsByMethod("shorejigging")}
            title="ショアジギングにおすすめの道具"
            description="ショアジギング入門に。メタルジグやタックルはこのあたりがおすすめです。"
            pageType="guide"
          />
        </section>
