...
import { getAutoGeneratedDiagram, getAnalysisResult } from "@/lib/patent/load-analysis";
import { isAnalysisLocationConsistent } from "@/lib/patent/normalize-analysis";
import { applyEnvironmentalEstimation, getMonthlyTopFish } from "@/lib/patent/fish-estimation";
...
async function getInitialCommunityPhotos(slug: string): Promise<{ url: string; uploadedAt: number }[]> {
  ...
  try {
    ...
    if (!photos) return [];
    return photos.map((p) => ({ url: p.url, uploadedAt: p.uploadedAt }));
  } catch {
    return [];
  }
}
...
export default async function SpotDetailPage({ params }: PageProps) {
  ...
  const rawAnalysisResult = getAnalysisResult(slug);
  const guardedAnalysisResult =
    rawAnalysisResult &&
    isAnalysisLocationConsistent(rawAnalysisResult, spot.latitude, spot.longitude)
      ? rawAnalysisResult
      : null;
  const autoGeneratedDiagram = guardedAnalysisResult ? getAutoGeneratedDiagram(slug) : null;
  ...
  const estimationCtx = guardedAnalysisResult
    ? {
        ...
        catchableFish: spot.catchableFish.map((cf) => ({
          name: cf.fish.name,
  ...
  const analysisResult =
    guardedAnalysisResult && estimationCtx
      ? applyEnvironmentalEstimation(guardedAnalysisResult, estimationCtx)
      : null;
  const monthlyTopFish =
    analysisResult && estimationCtx
      ? getMonthlyTopFish(analysisResult, estimationCtx, 5)
      : [];
  ...
  return (
    <div className="container mx-auto px-4 py-6 sm:py-8">
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          {isHiddenGem(spot) && (
            <Badge className="bg-emerald-600 hover:bg-emerald-600 text-xs">
              <Gem className="mr-0.5 size-3" />
              穴場スポット
            </Badge>
          )}
          {getPremiumFishForSpot(spot).length > 0 && (
            <Badge className="bg-amber-600 hover:bg-amber-600 text-xs">
      ...
      <section className="mb-6 sm:mb-8">
        <SpotPhotoGallery photos={spot.spotPhotos} spotType={spot.spotType} spotName={spot.name} spotSlug={spot.slug} latitude={spot.latitude} longitude={spot.longitude} initialCommunityPhotos={initialCommunityPhotos} />
        {/* SVG図: 一旦非表示 */}
        {/* {fishingPoints && (
          <div className="mt-4">
        ...
        {analysisResult && <LeafletMapSection
          ...
          spot={{
            ...
            isFree: spot.isFree,
            feeDetail: spot.feeDetail,
            managementInfo: spot.managementInfo ? {
              organizationName: spot.managementInfo.organizationName,
              contactPhone: spot.managementInfo.contactPhone,
              openingHours: spot.managementInfo.openingHours,
              closedDays: spot.managementInfo.closedDays,
              fishingFee: spot.managementInfo.fishingFee,
            } : undefined,
          }}
          spotFacilities={{
            hasParking: spot.hasParking,
            parkingDetail: spot.parkingDetail,
            parkingGuide: spot.parkingGuide ? {
              parkingLatitude: spot.parkingGuide.parkingLatitude,
              parkingLongitude: spot.parkingGuide.parkingLongitude,
            } : undefined,
            ...
            rentalDetail: spot.rentalDetail,
          }}
          restrictedAreas={spot.rules?.restrictedAreas}
          nearbySpots={allNearby.slice(0, 5).map((s) => ({
            slug: s.slug,
            name: s.name,
            latitude: s.latitude,
            longitude: s.longitude,
          ...
          <div className="mt-6">
            <SatelliteAnalysisSection
              ...
              monthlyRecommendation={
                monthlyTopFish.length > 0
                  ? {
                      month: currentMonth,
                      fish: monthlyTopFish.map((f) => ({
                        name: f.name,
                        method: f.method,
                        recommendLevel: f.recommendLevel,
                        catchReportCount: f.catchReportCount,
                        inSeasonNow: f.inSeasonNow,
                      })),
                    }
                  : undefined
              }
      ...
      <section className="mb-6">
        <SpotWeatherTide
          lat={spot.latitude}
          lng={spot.longitude}
          hideBestTimes={spot.isManagedPond || spot.slug === "fishing-park-hirano"}
          tideMode={getTideDisplayMode(spot)}
          tideStation={getTideStationForSpot(spot)}
        />
      </section>
