...
echo "       source $OUT"
echo "       # --body は付けないこと。gh secret set は「--body 未指定なら標準入力から読む」仕様で、"
echo "       # \`--body -\` と書くと値がハイフン1文字になりパイプが無視される（2026-07-15の障害の原因）。"
echo "       printf '%s' \"\$RUNTIME_AWS_ACCESS_KEY_ID\"     | gh secret set RUNTIME_AWS_ACCESS_KEY_ID     --repo thekkmasaki/tsurispot"
echo "       printf '%s' \"\$RUNTIME_AWS_SECRET_ACCESS_KEY\" | gh secret set RUNTIME_AWS_SECRET_ACCESS_KEY --repo thekkmasaki/tsurispot"
echo "  B) 登録後、鍵ファイルを消す:"
