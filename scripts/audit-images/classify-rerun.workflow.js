export const meta = {
  name: 'tsurispot-image-classify-rerun',
  description: '未処理の分類バッチのみ再実行（冪等・ファイル書き出し）',
  phases: [{ title: 'Classify', detail: '未処理バッチを視覚分類' }],
}

const ROOT = '/Users/masakiieyasu/saas-backup/tsurispot-fresh'
// 未処理バッチID（aggregate-classify で missing と判明したもの）
const MISSING_IDS = Array.from({ length: 35 }, (_, i) => String(202 + i).padStart(3, '0'))

const prompt = (id) => `釣りスポット情報サイト TsuriSpot の画像品質監査。バッチ ${id} を処理する。

手順:
1. まず Bash で \`test -f ${ROOT}/.audit-cache/results/classify-${id}.json && echo EXISTS || echo MISSING\` を実行。EXISTS なら処理済みなので「skipped ${id}」とだけ答えて終了。
...
const results = await parallel(
  MISSING_IDS.map((id) => () => agent(prompt(id), { label: `classify:${id}`, phase: 'Classify' }))
)
log(`再実行完了: ${results.filter(Boolean).length}/${MISSING_IDS.length} エージェント完了`)
