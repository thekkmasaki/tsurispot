...
## 監査反映: 移行前の必須確認（2026-07 総点検）

OOM障害後の総点検で判明した「Lightsailで顕在化する落とし穴」。上の各Phaseに織り込み済みだが要点を再掲。

- **ヒープ天井の持ち越し（対応済み）**: Dockerfileの`--max-old-space-size=1536`はLightsail 8GBでもそのまま。`gen-containers-json.sh`が`NODE_OPTIONS=--max-old-space-size=5120`を明示注入するよう修正済み（containers.jsonのenvがDockerfile ENVを上書き）。これでL1バイト上限(#249)と合わせ二重に安全。
- **AUTH_SECRET継続性（critical）**: 移行前後で**同値必須**。異なると全セッション失効＋`error=Configuration`。移設した`AUTH_SECRET`のGitHub Secretが現App Runner値(digest 69318d7d/len44)と一致するか確認。
- **認証cookie**: `__Host-`/`__Secure-` + `SameSite=Lax`。Cloudflareを**Full(strict)/HTTPS維持**し`AUTH_URL=https://tsurispot.com`を絶対に外さない。切替直後に`curl -D`で`Set-Cookie`に`Secure`が付くか確認。
- **AUTH_URL固定の制約**: 直URLでは実ログイン往復テスト不可。auth/Stripe検証はCloudflare切替後(Host=tsurispot.com)に実施。
- **Rekognition是正の確認**: 現インスタンスロールに権限が無くモデレーションが無言で死んでいる（fail-open）。移行IAMユーザー(`iam-runtime-policy.json`に`DetectModerationLabels`含む)適用後、**テスト画像で`safe=false`が返るか実地確認**。`[rekognition] moderation failed`ログが出ていないことも確認。
- **admin/push系のLBタイムアウト**: `maxDuration=300s`のadmin一括ルートはLightsail LB(~60s)で504。**LB経由で長時間ジョブを流さない**（コンテナ内直実行/短バッチ分割）。SSR(60s)もcold miss時にリスク→ISRウォーム維持。
- **origin lockdown**: 現状休眠(`ORIGIN_LOCKDOWN_MODE`未設定)。Lightsail直URLが新公開オリジンになるためCDNバイパスのコストリスク。`ORIGIN_VERIFY_SECRET`/`ORIGIN_LOCKDOWN_MODE`をSecretsに追加すれば`gen-containers-json.sh`が自動で含める（OPTIONAL passthrough）。**enforceは切替と別ステップ**にし、切替直後は`log`で運用しwarn 0を確認してから`enforce`へ。
